      ...
      return {
        select() {
          const builder: any = {
            ...
            single() {
              if (state.budget === undefined) return { data: null, error: { code: "PGRST116" } };
...
test("requires service role key", async () => {
  ...
  const handler = createHandler({ supabase, depositNanoUsd: 10, serviceRoleKey: "svc" });
...
test("deposits budget", async () => {
  ...
  const handler = createHandler({ supabase, depositNanoUsd: 10, serviceRoleKey: "svc" });
  const res = await handler(new Request("http://localhost", { method: "POST", headers: { authorization: "Bearer svc" }, body: JSON.stringify({ user_id: "u1" }) }));
